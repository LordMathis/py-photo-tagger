...
def process_image(img_path: str, input_queues: List[Queue], session: Session):
    try:
        ...
        with open(img_path, "rb") as f:
            ...
            photo = session.query(Photo).get(img_hash)
            ...
            if photo.latitude is None or photo.longitude is None or photo.city is None or photo.country is None:
            ...
            for queue in input_queues:
                queue.put((img_path, img_hash, image))
    except IOError:
...
class DatasetHandler(FileSystemEventHandler):
    ...
    def __init__(self, input_queues: List[Queue], db: Database):
    ...
    def on_any_event(self, event: FileSystemEvent):
        ...
        with self._db.session() as sess:
            process_image(event.src_path, self._input_queues, sess)


class DatasetWorker(threading.Thread):

    def __init__(self, input_queues: List[Queue], db: Database, thread_event: threading.Event):
        super().__init__()
        ...
        with self._db.session() as sess:
            for path, _, files in os.walk(DATA_BASE_PATH):
                for file in files:
                    ...
                    process_image(img_path, self._input_queues, sess)
